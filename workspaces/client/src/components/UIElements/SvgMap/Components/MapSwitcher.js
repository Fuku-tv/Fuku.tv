    ...
    <div className={`map-switcher-container ${props.usStateName ? 'three-col' : 'two-col'}`}>
      {props.usStateName && (
        <div className="state-container">
          <button onClick={stateHandler} className={`map-switcher-button ${props.activeMapRegion === 'STATE' && '--active'}`}>
            {stateName}
          </button>
          {props.activeMapRegion === 'STATE' && (
            <div className="change-state-button">{stateSelectIsActive && props.activeMapRegion === 'STATE' ? <SortUp /> : <SortDown />}</div>
          )}

          {stateSelectIsActive && (
            <div className="state-dropdown-container">
              {usaData.map((state) => (
                <span onClick={() => handleStateSelect(state.state)} class="state-option" id={state.state}>
                  {state.state}
                </span>
              ))}
            </div>
