	...
	useEffect(
		() => {
			...
			let newMarkers = props.markers.filter(
				(marker) =>
					marker.coordinates[0] >= LNG_MIN &&
					marker.coordinates[0] <= LNG_MAX &&
					marker.coordinates[1] <= LAT_MAX &&
					marker.coordinates[1] >= LAT_MIN
			);
							...
							return (
								<g key={geo.rsmKey + '-name'}>
									{cur &&
										...
										centroid[0] < -67 &&
										(
											Object.keys(offsets).indexOf(cur.id) === -1 ? <Marker
