...
const SvgMap = (props) => {
  ...
  const getLocation = () => {
    ...
    function reverseGeocodingWithGoogle(latitude, longitude) {
      fetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${longitude},${latitude}&key=${process.env.REACT_APP_GOOGLE_API_KEY}`)
        .then((res) => res.json())
        .then((response) => {
          console.log("User's Location Info: ", response.results[response.results.length - 2].address_components[0].long_name);
  ...
  const mapOfWorld = <MapOfWorld activeMapRegion={activeMapRegion} isMobile={props.isMobile} loadedPlaces={loadedPlaces} markers={markers} />;
  const mapOfUSA = <MapOfUsa activeMapRegion={activeMapRegion} isMobile={props.isMobile} loadedPlaces={loadedPlaces} markers={markers} />;
  const mapOfState = (
    <MapOfState activeMapRegion={activeMapRegion} usStateName={usStateName} isMobile={props.isMobile} loadedPlaces={loadedPlaces} markers={markers} />
  );
