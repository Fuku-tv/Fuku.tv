...
tape('authentication fails with wrong credentials', (t) => {
  ...
  if (process.env.NODE_ENV === 'development') {
    nock(`https://${signedRequest.host}`).post('/CreateGiftCard').reply(403, {});
  }
});
...
tape('createGiftCard', (t) => {
  ...
  if (process.env.NODE_ENV === 'development') {
    const responseBody = helpers.CreateGiftCardResponse(client.config.partnerId, sequentialId, amount, currencyCode);

    nock(`https://${signedRequest.host}`).post('/CreateGiftCard').reply(200, responseBody);
  }
});
...
tape('cancelGiftCard', (t) => {
  ...
  const { sequentialId, signedRequest } = client.createGiftCard('NA', amount, currencyCode, (error, result) => {
    ...
    if (process.env.NODE_ENV === 'development') {
      nock(`https://${signedRequest.host}`)
        .post('/CancelGiftCard')
        .reply(200, helpers.CancelGiftCardResponse(client.config.partnerId, sequentialId, result.gcId));
    }
  ...
  if (process.env.NODE_ENV === 'development') {
    nock(`https://${signedRequest.host}`)
      .post('/CreateGiftCard')
      .reply(200, helpers.CreateGiftCardResponse(client.config.partnerId, sequentialId, amount, currencyCode));
  }
});
